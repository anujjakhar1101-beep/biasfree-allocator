...
import {
  LayoutDashboard, Users, PlusCircle, LogOut, BrainCircuit, ChevronLeft, ChevronRight, Menu
} from "lucide-react";

const navItems = [
  { label: "Dashboard", icon: LayoutDashboard, path: "/manager/dashboard" },
  { label: "Employees", icon: Users, path: "/manager/employees" },
  { label: "Add Project", icon: PlusCircle, path: "/manager/add-project" },
];
      ...
      <div className={`flex items-center gap-3 px-4 py-5 border-b ${collapsed ? "justify-center" : ""}`} style={{ borderColor: "hsl(var(--sidebar-border))" }}>
        ...
        {!collapsed && (
          <div>
            ...
            <div className="text-xs" style={{ color: "hsl(var(--sidebar-foreground))", opacity: 0.5 }}>Manager Portal</div>
          </div>
        )}
      </div>
      ...
      {!collapsed && (
        <div className="px-4 py-4 border-b" style={{ borderColor: "hsl(var(--sidebar-border))" }}>
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold text-white flex-shrink-0" style={{ background: "hsl(38 92% 50%)" }}>
              {user?.name?.charAt(0)}
            </div>
            <div className="min-w-0">
              <div className="text-sm font-semibold text-white truncate">{user?.name}</div>
              <div className="text-xs" style={{ color: "hsl(var(--sidebar-foreground))", opacity: 0.55 }}>Project Manager</div>
            </div>
          </div>
        </div>
      )}
